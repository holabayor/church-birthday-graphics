...
type LeadershipRole = Extract<UnitRole, "head" | "assistant">;

const leadershipRoles: Array<{ role: LeadershipRole; label: string }> = [
  { role: "head", label: "HOD / Head" },
...
export function UnitLeadershipStack({ members }: { members: UnitMember[] }) {
  return (
    <div className="space-y-2">
      {leadershipRoles.map(({ role, label }) => {
        ...
        return (
          <div key={role} className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-[#0B1C30]">
              {leader ? getUnitMemberName(leader) : role === "head" ? "No HOD assigned" : "No assistant assigned"}
            </span>
...
export function UnitLeaderCard({
  ...
  const leader = getUnitLeader(members, role);
  const label = role === "head" ? "HOD / Head" : "Assistant";
