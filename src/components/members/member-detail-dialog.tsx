  ...
  return (
    ...
    >
      <CollapsibleTrigger className="flex w-full items-center justify-between font-sans font-semibold text-base md:text-lg cursor-pointer p-4 bg-[var(--surface-container-lowest)] hover:bg-[var(--surface-container-low)] transition-colors select-none text-left">
        ...
        <ChevronDown
          className={`h-5 w-5 text-[var(--outline)] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </CollapsibleTrigger>
      <CollapsibleContent className="border-t border-[var(--outline-variant)]/40 bg-[var(--surface-container-lowest)]">
        {children}
      </CollapsibleContent>
    </Collapsible>
  );
          ...
          <div className="flex flex-col md:flex-row h-full overflow-hidden relative">
            ...
            <div className="w-full md:w-[35%] shrink-0 relative bg-[var(--surface-container-low)] border-r border-[var(--outline-variant)]/20 hidden md:block">
              ...
              <div className="absolute bottom-0 w-full bg-gradient-to-t from-[var(--inverse-surface)]/90 via-[var(--inverse-surface)]/60 to-transparent p-6 pt-20">
                ...
                {member.created_at && (
                  <p className="font-sans text-[var(--inverse-on-surface)]/80 text-sm">
                    Joined{" "}
            ...
            <div className="w-full md:w-[65%] flex flex-col h-full bg-[var(--surface-container-lowest)] overflow-y-auto">
              ...
              <div className="p-6 md:p-10 flex-1 space-y-8">
                ...
                <div>
                  ...
                  <DialogDescription className="font-mono text-xs text-[var(--outline)] mt-1">
                    ID: {member.id}
                  </DialogDescription>
                </div>

                {/* Action Buttons */}
                <div className="flex flex-wrap items-center gap-3 pb-6 border-b border-[var(--outline-variant)]/30">
                  {member.phone_number && (
                    <Button
                      onClick={() => window.open(`tel:${member.phone_number}`)}
                      className="flex-grow sm:flex-none inline-flex items-center justify-center gap-2 px-5 h-10 bg-primary text-primary-foreground rounded-lg font-semibold text-sm hover:bg-primary/95 transition-colors shadow-sm"
                    >
                ...
                <div className="space-y-4">
                  ...
                  {canSeeOperational && timelineItems.length > 0 && (
                    <CollapsibleSection title="Church History" icon={Clock3} iconColorClass="text-primary">
                      <div className="p-5">
                        <div className="relative pl-6 border-l-2 border-[var(--surface-dim)] space-y-6">
                  ...
                  {canSeeFull && (
                    <CollapsibleSection
                      ...
                      icon={Clock3}
                      iconColorClass="text-[var(--outline)]"
                    >
                      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6 text-sm">
                        <div>
                          <p className="font-mono text-[10px] uppercase tracking-wider text-[var(--outline)] mb-0.5">
